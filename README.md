...
## Local (no Docker)
1. `npm i`
2. Copy `.env.example` to `.env` and fill in the values (the sample lists the defaults for `MONGODB_URI`, `PORT`, and `NODE_ENV`)
3. Ensure MongoDB is running (default `mongodb://localhost:27017`)
4. `npm run dev` (or `npm start`)
5. Access the bundled frontends via the API server (see “Frontends” below)
...
## Frontends
- `GET http://localhost:4000/storefront/` (shop UI powered by `storefront/shop.js`, which uses `/api` for data)
- `GET http://localhost:4000/pos/pos.html` (POS interface; all requests go to the same API host, so you only need the backend running)
- `GET http://localhost:4000/backoffice/backoffice.html` (staff dashboard)
All of the above are served from the Express app, so once `npm run dev` is running you can open the URLs in the browser without an extra static server.


## Endpoints
- `GET /api/products` (q, category, page, limit, sort, dir)
- `POST /api/products`
- `PUT /api/products/:id`
- `DELETE /api/products/:id`
- `PATCH /api/products/:id/stock`
- `GET /api/products/duplicates/by-ean`
- `POST /api/products/bulk` (JSON array)
